...
mock.onGet(/\/api\/user-management\/user\/\d+/).reply((request) => {
...
// POST : Add new Item
mock.onPost("/api/user-management/user/add").reply((request) => {
...
// Update Product
mock.onPost("/api/user-management/user/update").reply((request) => {

  let reqItem = JSON.parse(request.data).item
console.log(reqItem)
  let item = data.users.find((item) => item.id == reqItem.id)
  console.log(item)
  Object.assign(item, reqItem)
  console.log(data.users)
  return [200, item]
})

// // DELETE: Remove Item
mock.onDelete(/\/api\/user-management\/user\/\d+/).reply((request) => {
