...
<template>
    <div v-if="userDetails != null" class="chat__header">
        <vs-navbar class="p-4 flex navbar-custom" color="white" type="flat">
            <div class="relative flex mr-4">
                ...
                <vs-avatar class="m-0 border-2 border-solid border-white" size="40px" :src="userImg(userDetails)" @click.stop="$emit('showProfileSidebar', userDetails.id)" />
    ...
    <div v-else class="chat__header">
        <vs-navbar class="p-4 flex navbar-custom" color="white" type="flat">
            <div class="relative flex mr-4">
                <feather-icon icon="MenuIcon" class="mr-4 cursor-pointer"  v-if="isSidebarCollapsed" @click.stop="$emit('openContactsSidebar')" />
                <vs-avatar class="m-0 border-2 border-solid border-white" size="40px" :src="userImg(this.$store.state.AppActiveUser)" />
                <div class="h-3 w-3 border-white border border-solid rounded-full absolute right-0 bottom-0" :class="'bg-' + getStatusColor(false)"></div>
            </div>
...
export default {
  ...
  computed: {
    ...
    userDetails () {
      let chatSession = this.$store.getters['chat/chatSession'](this.chatSessionId)
      return chatSession?chatSession.agentInfo:{
        id:0,
        profileUrl:'',
        userName:''
      }
    ...
    customer () {
      let chatSession = this.$store.getters['chat/chatSession'](this.chatSessionId)
      return chatSession?chatSession.customerInfo:{
        customerId:''
      }
    },
    getStatusColor () {
