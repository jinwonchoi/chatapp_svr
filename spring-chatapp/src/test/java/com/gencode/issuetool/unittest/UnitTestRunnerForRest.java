...
public class UnitTestRunnerForRest {
	LogWrapper logger = new LogWrapper(UnitTestRunnerForRest.class);
	
    TestData testData; 
    String token;
    
	public UnitTestRunnerForRest() {
	}
    ...
    public void runLoginAndDoCrudNoticeBoard() {
    	try {
			...
			//* 2. 등록 1
    		NoticeBoard notice1 = new NoticeBoard("테스트 공지사항 1", "test notice content", 1/*admin*/,"N", 0, 0);
			...
			//* 6. 등록 2
    		NoticeBoard notice2 = new NoticeBoard("테스트 공지사항 2", "test notice content2", 1/*admin*/,"N", 0, 0);
	...
	PageResultObj<List<NoticeBoardEx>> _callListPageNotice(String testTitle, boolean flag) {
		...
		Map<String, String> map = new HashMap<String, String>();
		PageRequest req = new PageRequest(1, 10, 0, SortDirection.ASC, "id", map, map);
